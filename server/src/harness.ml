...
(* unix sockets must be under ~100 characters long, so we create [socket_dir]
   in /tmp *)
let socket_dir =
  lazy
    (Filename_unix.temp_dir ~in_dir:"/tmp" "postgres-async-test-harness" "")
;;
...
let create ?(extra_server_args = []) () =
  ...
  let get_postgres_output () =
    In_channel.read_all postgres_output_filename
  in
  (* Ask the OS to assign us a temporary port. *)
  let temp_socket =
    Unix.socket ~domain:PF_INET ~kind:SOCK_STREAM ~protocol:0 ()
  in
  Unix.bind
    temp_socket
    ~addr:(ADDR_INET (Unix.Inet_addr.of_string "0.0.0.0", 0));
  let port =
  ...
  (* Postgres binds with SO_REUSEADDR.
     ...
     By binding the same port on [127.0.0.2] and _then_ switching REUSEADDR
     on [temp_socket2] off, it will not be possible to bind to
     [0.0.0.0:port], since such a bind will conflict with the
     [127.0.0.2:port] socket. In particular, another instance of this test
     will not re-use the port. But also other processes asking for
     [0.0.0.0:ephemeral] will not be given [port].

     A process asking for [127.0.0.1:ephemeral] without REUSEADDR set be
     handed our port.

     A process asking for an ephemeral port on [127.0.0.1] with REUSEADDR set
     could possibly be handed our port. It's very unlikely that any process
     actually attempts this. *)
  let temp_socket2 =
    Unix.socket ~domain:PF_INET ~kind:SOCK_STREAM ~protocol:0 ()
  in
  ...
  Unix.bind
    temp_socket2
    ~addr:(ADDR_INET (Unix.Inet_addr.of_string "127.0.0.2", port));
      ...
      (match Signal_unix.send Signal.quit (`Pid server_pid) with
       ...
       | `Ok ->
         (match Unix.waitpid_exn server_pid with
          ...
          | exception exn ->
            eprintf
              !"in at-exit handler, waiting for postgres failed: %{Exn}\n%!"
              exn));
      let pid =
        Unix.fork_exec
          ()
        ...
        (match Unix.wait_nohang (`Pid server_pid) with
         ...
         | None ->
           ...
           (match
              String.is_substring
                output
                ~substring:"ready to accept connections"
            with
            | false ->
...
let sighup_server { server_pid; _ } =
  Signal_unix.send_exn Signal.hup (`Pid server_pid)
;;

let unix_socket_path { socket_dir; port; _ } =
  sprintf "%s/.s.PGSQL.%i" socket_dir port
;;
